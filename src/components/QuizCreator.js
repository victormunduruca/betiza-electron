...
export default function QuizCreator(props) {
    ...
    if (!props.create) {
        ...
        startingQuestions = [{ key: uuid(), stimulus: "", stimulusType: "text", correctAnswerKey: "", answers: [], answerType: "text" }];
    }
    ...
    function addQuestion() {
        if (isCorrectKeyFilled(selectedQuestionKey)) { //if current question validated, create new question
            //clear error mode
            setErrorState(false);
            let newQuestionKey = uuid();
            //create new question component
            setQuestions(prevQuestions => {
                return [...prevQuestions, { key: newQuestionKey, stimulus: "", stimulusType: "text", correctAnswerKey: "", answers: [], answerType: "text" }]; //Add new empty question to questions
            })
        } else {
            console.log("Please fill correct answer field");
            setErrorState(true);
            //put checkboxes in error mode
            //show error
        }
        //show modal with warning in case the user didn't set the correct answer

    }

    function isCorrectKeyFilled(questionKey) {
        console.log("QUESTION CORRECT ANSWER:" + getQuestion(questionKey).correctAnswerKey);
        return getQuestion(questionKey).correctAnswerKey != "";
    }
